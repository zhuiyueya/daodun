...
import supportLogoTokenMap from './assets/词元映射.png'
import {
  ApiError,
  approveWork,
  createWork,
...
const homeLandingPageHtml = landingPageHtml
  ...
  .replaceAll('./src/assets/词元映射.png', supportLogoTokenMap)
...
function AuthPage({
  ...
  async function handleRequestCode() {
    const allowedDomains = ['163.com', 'qq.com', 'gmail.com', 'foxmail.com']
    const domain = email.split('@')[1]?.toLowerCase()
    if (!domain || !allowedDomains.includes(domain)) {
      onNotice('仅支持 163、QQ、Gmail、Foxmail 邮箱登录')
      return
    }
    try {
      setSubmitting(true)
      ...
      <article className="detail-card form-card">
        ...
        <label className="field">
          ...
          <input
            ...
            type="email"
            placeholder="支持 163、QQ、Gmail、Foxmail 邮箱"
          />
        </label>
        {sent ? (
          <label className="field">
